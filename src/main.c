...
static void instruction_exec(uint32_t size) {
  for (reg_pc = (uint16_t *)(memory + 0x200);
    ...
    case 0x0C: {
      ...
      regs[x] =
          rand & kk_byte; // TODO: Swap bitwise and with the CHIP8 AND instr
      reg_pc++;

    } break;
    case 0x0D: {
      printf("Display %d-byte sprite starting at memory location I at (%d, "
             "%d), set VF = collision.\n",
    ...
    case 0x0E: {
      ...
      case 0x9E: {
        printf(
            "Skip next instruction if key with the value of V%d is pressed.\n",
            x);
        if (keypad[x])
          reg_pc += 2;
        else
      ...
      case 0xA1: {
        printf("Skip next instruction if key with the value of V%d is not "
               "pressed.\n",
               x);
        if (!keypad[x])
          reg_pc += 2;
    ...
    case 0x0F: {
      ...
      case 0x33: {
        printf("Store BCD representation of V%d in memory location I, I + 1 "
               "and I + 2.\n",
               x);
      ...
      case 0x55: {
        printf("Store registers V0 through V%d in memory starting at location "
               "I.\n",
               x);
      ...
      case 0x65: {
        printf(
            "Read registers V0 through V%d in memory starting at location I.\n",
            x);
        for (uint64_t i = 0; i < x; i++) {
...
static void init() { memcpy(memory, sprites, sizeof sprites); }
